...
> ```bash
> # macOS / Linux 用户复制这行：
> for name in IFLY_APP_ID IFLY_API_KEY IFLY_API_SECRET; do
>   if printenv "$name" | grep -q .; then echo "$name: set"; else echo "$name: missing"; fi
> done
>
> # Windows 用户在 cmd 里运行这行：
> for %V in (IFLY_APP_ID IFLY_API_KEY IFLY_API_SECRET) do @if defined %V (echo %V: set) else (echo %V: missing)
> ```
>
> 上述命令仅检查是否已配置，不打印凭证值。显示 missing 时需要配置对应变量；显示 set 后仍须确认三项来自同一应用。设置方法：
